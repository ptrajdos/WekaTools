...
public class SimpsonsIntegrator extends SimpleIntegrator {
	...
	public double integrate() {
		...
		double tmpVal =0;
		value += this.delta/3.0 * (this.getFunction().getValue(sequence[0]) +
				this.getFunction().getValue(sequence[sequence.length-1]));
		
		for(int i=1;i<sequence.length -1;i++) {
			if(i%2==0) {
				tmpVal= 2.0*this.getFunction().getValue(sequence[i]);
			}else {
				tmpVal= 4.0*this.getFunction().getValue(sequence[i]);
			}
